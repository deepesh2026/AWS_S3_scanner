...
${toHex(hashedRequest)}`;
  ...
  var package_default = {
    ...
    devDependencies: {
      "@aws-sdk/signature-v4-crt": "3.1039.0",
  ...
  async function checkPolicy(bucketName) {
    ...
    try {
      ...
      console.error("Failed to parse policy", e2);
    }
  ...
  function generateReport(auditResults2) {
    let totalChecks = 0;
    let passedChecks = 0;
    let failedChecks = 0;
    let warnings = 0;
    ...
    container.innerHTML = "";
    auditResults2.forEach((bucketData) => {
      let bucketPassed = 0;
      let bucketFailed = 0;
      let bucketWarn = 0;
      let bucketRisk = "LOW";
      ...
      const hasHighRiskFail = bucketData.checks.some((c2) => c2.status === "FAIL" && c2.severity === "HIGH");
      if (hasHighRiskFail) {
        bucketRisk = "HIGH";
        badgeClass = "badge-risk-high";
        riskLabel = "HIGH RISK \u{1F534}";
      } else if (bucketFailed > 0 || bucketWarn > 0) {
        bucketRisk = "MEDIUM";
        badgeClass = "badge-risk-medium";
        riskLabel = "MED RISK \u{1F7E1}";
      } else {
        riskLabel = "SECURE \u{1F7E2}";
      }
      const cardHtml = `
            <div class="bucket-card">
                <div class="bucket-header">
                    <div class="bucket-name">\u{1F4E6} <i>${bucketData.bucketName}</i></div>
    ...
    document.getElementById("totalBucketsCount").textContent = auditResults2.length;
    ...
    setupExportButton(auditResults2);
  }
  ...
  var credentialsScreen = document.getElementById("credentialsScreen");
  var scanningScreen = document.getElementById("scanningScreen");
  var reportScreen = document.getElementById("reportScreen");
  var headerStatus = document.getElementById("headerStatus");
  var activeRegionBadge = document.getElementById("activeRegionBadge");
  var disconnectBtn = document.getElementById("disconnectBtn");
  var scanLogFeed = document.getElementById("scanLogFeed");
  ...
  authForm.addEventListener("submit", async (e2) => {
    e2.preventDefault();
  ...
  async function testConnection() {
    ...
    try {
      ...
      for (let i2 = 0; i2 < buckets.length; i2++) {
        ...
        log(`Completed ${checks.length} checks for ${b2.Name}.`);
      }
